...
<!--      <el-table-column label="是否启用" align="center" prop="enable" />-->
      <el-table-column label="备注" align="center" prop="commont" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="类别备注" prop="commont">
              <el-input v-model="form.commont" placeholder="请输入类别备注" />
            </el-form-item>
          </el-col>
        </el-row>
  ...
  methods: {
    ...
    handleAdd() {
      ...
      listCategory().then(response => {
        this.categoryOptions = this.handleTree(response.rows, 'id');
      })
    ...
    handleUpdate(row) {
      ...
      getCategory(id).then(response => {
        ...
        this.open = true;
        this.title = "修改类别管理";
        getcategoryexcludechrild(id).then(response=>{
          this.categoryOptions = this.handleTree(response.data, 'id');
          if (this.categoryOptions.length == 0) {
            const noResultsOptions = { id: this.form.parentId, name: this.form.name, children: [] };
