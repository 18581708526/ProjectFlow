...
import {aichatreq} from "@/api/chat/chat"
  ...
  methods: {
    async sendMessage() {
      ...
      try {
        ...
        aichatreq(this.inputMessage).then(response=>{
          console.log(response);
          // 添加AI回复
          const aiMessage = {
            content: response, // 根据你的接口返回结构调整
            sender: 'ai',
            time: this.getCurrentTime()
          };
...
<!--      try {-->
<!--        const response = await aichatreq(this.inputMessage);-->
<!--        const aiMessage = {-->
<!--          content: response,-->
<!--          sender: 'ai',-->
<!--          time: this.getCurrentTime()-->
...
export default {
  ...
  methods: {
    ...
    async getResponse(message) {
      try {
        const response = await fetch(`http://localhost:8088/ai/chat?message=${encodeURIComponent(message)}`);
        if (!response.ok) {
