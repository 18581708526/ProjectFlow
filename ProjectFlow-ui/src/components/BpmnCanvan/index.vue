...
import {getModelResource, updateModelNew} from "@/api/model/model";
export default {
  props: {
    modelId: {
      type: String,
  ...
  methods: {
    getModelXml(){
      getModelResource(this.modelId).then(response => {
    ...
    saveModeler(data) {
      console.log(data);
      this.modelVo=data;
      this.modelVo.modelId=this.modelId;
      updateModelNew(this.modelVo).then(response => {
